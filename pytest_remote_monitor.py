...
def test_gomukh_controller(dut: IdfDut, log_minimum_free_heap_size: Callable[..., None]) -> None:
...
def test_gomukh_controller_linux(dut: IdfDut) -> None:
    dut.expect('Hello world!')
...
@idf_parametrize('target', ['linux'], indirect=['target'])
def test_gomukh_controller_macos(dut: IdfDut) -> None:
    dut.expect('Hello world!')
...
def test_gomukh_controller_host(app: QemuApp, dut: QemuDut) -> None:
